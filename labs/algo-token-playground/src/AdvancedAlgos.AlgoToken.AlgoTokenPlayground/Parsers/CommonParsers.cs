...
namespace AdvancedAlgos.AlgoToken.AlgoTokenPlayground.Parsers
{
    public static class CommonParsers
    {
        ...
        public static readonly Parser<DateTime> DateValue =
            from year in Integer.Select(x => int.Parse(x, CultureInfo.InvariantCulture))
            from _1 in Parse.Char('-')
            from month in Integer.Select(x => int.Parse(x, CultureInfo.InvariantCulture))
            from _2 in Parse.Char('-')
            from day in Integer.Select(x => int.Parse(x, CultureInfo.InvariantCulture))
            from T in Parse.Char('T')
            from hour in Integer.Select(x => int.Parse(x, CultureInfo.InvariantCulture))
            from _3 in Parse.Char(':')
            from minute in Integer.Select(x => int.Parse(x, CultureInfo.InvariantCulture))
            from _4 in Parse.Char(':')
            from second in Integer.Select(x => int.Parse(x, CultureInfo.InvariantCulture))
